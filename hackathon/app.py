...
INDICES: List[Dict[str, Any]] = [
    # Global popular indices (Yahoo Finance symbols)
    {"symbol": "^GSPC", "name": "S&P 500 (^GSPC)", "category": "global", "constituents": []},
    {"symbol": "^DJI", "name": "Dow Jones Industrial Average (^DJI)", "category": "global", "constituents": []},
    {"symbol": "^NDX", "name": "Nasdaq-100 (^NDX)", "category": "global", "constituents": []},
    {"symbol": "^FTSE", "name": "FTSE 100 (^FTSE)", "category": "global", "constituents": []},
    {"symbol": "^GDAXI", "name": "DAX (^GDAXI)", "category": "global", "constituents": []},
    {"symbol": "^N225", "name": "Nikkei 225 (^N225)", "category": "global", "constituents": []},
    {"symbol": "^HSI", "name": "Hang Seng Index (^HSI)", "category": "global", "constituents": []},

    # Sector ETFs (classified)
    {"symbol": "XLK", "name": "Technology Sector ETF (XLK)", "category": "sector_etf", "constituents": ["AAPL","MSFT","NVDA","AVGO","ADBE","AMD","CRM"]},
    {"symbol": "XLF", "name": "Financials Sector ETF (XLF)", "category": "sector_etf", "constituents": ["JPM","BAC","WFC","GS","MS","SCHW","C"]},
    {"symbol": "XLE", "name": "Energy Sector ETF (XLE)", "category": "sector_etf", "constituents": ["XOM","CVX","COP","SLB","EOG","PSX","MPC"]},
    {"symbol": "XLV", "name": "Health Care Sector ETF (XLV)", "category": "sector_etf", "constituents": ["UNH","JNJ","LLY","ABBV","MRK","PFE","TMO"]},
    {"symbol": "XLY", "name": "Consumer Discretionary Sector ETF (XLY)", "category": "sector_etf", "constituents": ["AMZN","TSLA","MCD","NKE","SBUX","BKNG"]},
    {"symbol": "XLP", "name": "Consumer Staples Sector ETF (XLP)", "category": "sector_etf", "constituents": ["PG","KO","PEP","WMT","COST","MDLZ"]},
    {"symbol": "XLU", "name": "Utilities Sector ETF (XLU)", "category": "sector_etf", "constituents": ["NEE","DUK","SO","D","AEP","EXC"]},
    {"symbol": "XLI", "name": "Industrials Sector ETF (XLI)", "category": "sector_etf", "constituents": ["HON","UNP","RTX","CAT","LMT","GE"]},
    {"symbol": "IYR", "name": "Real Estate Sector ETF (IYR)", "category": "sector_etf", "constituents": ["AMT","PLD","SPG","EQIX","WELL","O"]},
]
...
def portfolio_diagnostic():
    try:
        ...
        if sector_conc > 0.70:
            analysis['main_issues'].append('Sector concentration is too high')
            analysis['suggestions'].append('Add defensive sectors like Consumer Staples, Health Care, and Utilities')
        elif sector_conc > 0.50:
            analysis['main_issues'].append('Sector concentration is elevated')
            analysis['suggestions'].append('Introduce cross-sector holdings to reduce correlation')

        if single_stock_weight > 0.30:
            analysis['main_issues'].append('Single-stock weight is too high')
            analysis['suggestions'].append('Cap any single holding at no more than 20% of portfolio')
        elif single_stock_weight > 0.20:
            analysis['main_issues'].append('Max single-stock weight is elevated')
            analysis['suggestions'].append('Increase smaller positions to improve diversification')

        if (avg_pe or 0) > 25:
            analysis['main_issues'].append('Overall valuation is high')
            analysis['suggestions'].append('Consider lower-valuation value stocks or dividend assets to balance')
        elif (avg_pe or 0) > 20:
            analysis['main_issues'].append('Valuation is on the higher side')
            analysis['suggestions'].append('Moderately reduce growth exposure; add defensive or stable cash-flow names')

        issues = ', '.join(analysis['main_issues']) if analysis['main_issues'] else 'Overall structure is balanced'
        suggestions = '; '.join(analysis['suggestions']) if analysis['suggestions'] else 'Maintain current allocation; monitor fundamentals'
        summary_text = (
            f"Diversity Score {diversity_score}, Risk Level {risk_level}; "
            f"Max single-stock weight {round(single_stock_weight*100,1)}%, Sector concentration {round(sector_conc*100,1)}%, "
            f"Weighted Avg PE {avg_pe if avg_pe is not None else '-'}.\n"
            f"Main issues: {issues}. Suggestions: {suggestions}."
        )
...
def ai_analysis():
    ...
    prompt = (
        f"As an investment advisor, analyze {symbol}:\n\n"
        ...
        "Provide a concise investment recommendation including:\n"
        "1. Technical analysis\n"
        "2. Risk warnings\n"
        "3. Actionable suggestion\n\n"
        "Requirements: professional yet easy to understand, within 150 words."
    )
    sys_msg = "You are a professional financial analyst who explains complex concepts in plain English."
...
def _local_ai_analysis(symbol: str, current_price: Any, change_percent: Any, pe_ratio: Any, rsi14: Any) -> str:
    """Simplified local analysis based on basic indicators, about 150 words."""
    try:
        price = current_price
        chg = change_percent
        pe = pe_ratio
        ...
        if rsi is not None:
            try:
                r = float(rsi)
                if r <= 30:
                    tech.append("RSI low; strong oversold conditions")
                elif r >= 70:
                    tech.append("RSI high; overbought risk")
                else:
                    tech.append("RSI neutral")
            except Exception:
                tech.append("RSI data limited")
        if chg is not None:
            try:
                c = float(chg)
                if c >= 2:
                    tech.append("Short-term upward momentum is strong")
                elif c <= -2:
                    tech.append("Short-term pullback pressure is notable")
            except Exception:
                pass
        if tech:
            parts.append("Technical: " + ", ".join(tech))
        # 风险
        risk = []
        if pe is not None:
            try:
                p = float(pe)
                if p >= 30:
                    risk.append("Valuation is high; volatility may amplify")
                elif p <= 10:
                    risk.append("Valuation is low; fundamentals need confirmation")
            except Exception:
                pass
        risk.append("Mind event risk and market environment")
        parts.append("Risk: " + ", ".join(risk))
        # 操作建议
        op = []
        try:
            r = float(rsi) if rsi is not None else None
            if r is not None and r <= 30:
                op.append("Start with a small position; scale in gradually")
            elif r is not None and r >= 70:
                op.append("Trim positions cautiously; wait for a pullback")
            else:
                op.append("Stay on the sidelines; watch support and volume")
        except Exception:
            op.append("Prioritize risk control; set stop-loss levels")
        parts.append("Action: " + ", ".join(op))
        text = "; ".join(parts)
        # 控制长度
        return text[:160]
    except Exception:
        return "Limited data; consider waiting and manage risk."
